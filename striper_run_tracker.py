...
def compute_probability(grid, lats, lons, front_lines, sight_pts, chl_grid, weights):
    """Weighted blend of available 0-1 factors -> normalized 0-1 grid (NaN land).

    Returns (prob, used, detail, wnorm) where ``detail`` holds each factor's 0-1
    score grid plus raw inputs (SST/distances) for the click-to-inspect popups,
    and ``wnorm`` is the renormalized weight per used factor.
    """
    water = ~np.isnan(grid)
    scores, detail = {}, {}
    scores["temp"] = temp_suitability(grid)
    detail["temp"] = {"score": scores["temp"]}
    n_score, shore_km = nearshore_factor(grid, lats, lons)
    scores["near"] = n_score
    detail["near"] = {"score": n_score, "dist_km": shore_km}
    fr = front_factor(grid, lats, lons, front_lines)
    if fr is not None:
        scores["front"] = fr[0]
        detail["front"] = {"score": fr[0], "dist_km": fr[1]}
    sd = sighting_density(grid, lats, lons, sight_pts)
    if sd is not None:
        scores["sight"] = sd
        detail["sight"] = {"score": sd}
    ...
    if mx and mx > 0:
        prob = prob / mx
    print(f"[prob] blended factors: {', '.join(used)}", file=sys.stderr)
...
def _png_overlay(lats, lons, rgba):
    ...
    buf = io.BytesIO()
    mimg.imsave(buf, img[ri, :, :], format="png")
...
def _render_field(field, lats, lons, mask, cmap, norm, alpha_fn):
    ...
    a[np.isnan(fine)] = 0.0
    rgba[..., 3] = a
    return _png_overlay(mlats, mlons, rgba)


def render_prob_png(prob, lats, lons, mask):
...
def build_map(*, prob_uri, prob_bounds, sst_uri, sst_bounds, sst_vmin_f, sst_vmax_f,
              front_50, history, buoys, sightings, records, status, sparkline_uri,
              used, cells, insp_w, half, bbox, date_str, region, out_path):
    import folium
    ...
    # SST first (toggle), probability on top (headline).
    ImageOverlay(image=sst_uri, bounds=sst_bounds, opacity=0.65,
                 name="SST heatmap", show=False).add_to(m)
    ImageOverlay(image=prob_uri, bounds=prob_bounds, opacity=1.0,
                 name="Fish probability", show=True).add_to(m)
...
def main(argv=None):
    ...
    history = fetch_sst_history(bbox, args.history_days, date_str) if args.history_days > 0 else []
    ...
    prob, used, detail, wnorm = compute_probability(grid, lats, lons, front_50,
                                                    sight_pts, chl_grid, PROB_WEIGHTS)
    prob_uri, prob_bounds = render_prob_png(prob, lats, lons, mask)
    cells, half = build_inspection_cells(lats, lons, grid, prob, detail, mask)
    insp_w = {k: round(wnorm[k], 3) for k in ("temp", "near", "front", "sight")
              if k in wnorm}
    print(f"[inspect] {len(cells)} clickable cells", file=sys.stderr)

    home = (args.home_lat, args.home_lon, args.home_name)
    status = compute_status(buoys, history, front_50, grid, lats, lons, home, date_str)
    status.best_text = best_odds_text(prob, lats, lons, buoys, home)
    ...
    build_map(prob_uri=prob_uri, prob_bounds=prob_bounds, sst_uri=sst_uri,
              sst_bounds=sst_bounds, sst_vmin_f=sst_vmin_f, sst_vmax_f=sst_vmax_f,
              front_50=front_50, history=history, buoys=buoys, sightings=sightings,
              records=records, status=status, sparkline_uri=sparkline_uri, used=used,
              cells=cells, insp_w=insp_w, half=half, bbox=bbox, date_str=date_str,
